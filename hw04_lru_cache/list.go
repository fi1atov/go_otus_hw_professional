...
func (l *list) PushFront(v interface{}) *ListItem {
	...
	if l.last == nil {
		l.last = &item
	} else { // Иначе, если это не первый вставляемый элемент - нам нужно обратиться к этому элементу
		l.first.Prev = &item // и добраться до параметра Prev, чтобы установить ему наш новый элемент
	}
