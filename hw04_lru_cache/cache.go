...
func (c *lruCache) Clear() {
	...
	// Тут все таки лучше использовать make и сразу задать размер мамы, раз мы его знаем
	c.items = make(map[Key]*ListItem, c.capacity)
	// Очередь тоже нужно очистить, но это уже сложнее. Нужно избежать утечки памяти
	// Удалять буду от последнего элемента к первому
	elem := c.queue.Back() // получаем последний элемент
	for elem != nil {
		elem = elem.Prev               // в дальнейшем последним элементом будет считаться текущий предпоследний
		c.queue.Remove(c.queue.Back()) // удаляем последний
	}
}
